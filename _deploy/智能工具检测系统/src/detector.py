...
class ToolDetector:
    def __init__(self, tools, detector=None):
        ...
        self.ratio = RATIO
        # 诊断用：最近一次 detect 的有效阈值，以及每个识别到的工具的命中来源
        #   "强" = 内点 ≥ 阈值；"弱" = 弱匹配通道（≥5 且紧凑）；"色" = 钢丝钳颜色旁路
        self.last_threshold = INLIER_THRESHOLD
        self.last_source = {}
    ...
    def detect(self, frame_bgr):
        ...
        if any(t["name"] == PLIERS_NAME for t in self.tools):
            ...
            if score >= PLIERS_COLOR_THRESHOLD:
                ...
                # 颜色旁路命中：覆盖来源标记（即便 SIFT 也弱命中，这里以"色"为准——便于诊断）
                source[PLIERS_NAME] = "色"

"""Tkinter GUI：实时扫描模式（SIFT + RANSAC 检测后端，3 秒滑动窗口防抖）
...
class ToolDetectionApp:
        ...
        try:
            ...
            for tool in recognized + unrecognized:
                ...
                if match_label is not None:
                    ...
                    # count 是 SIFT 的 RANSAC 内点数
                    match_label.config(text=f"{count}" if count > 0 else "--")
                    if is_recog:
                        if count >= 10:
                            match_label.config(fg="#00ff88")
                        elif count >= 5:
                            match_label.config(fg="#ffcc00")
                        else:
                            match_label.config(fg="#888888")
                    else:
                        match_label.config(fg="#555555")

            total = len([t for t in self.tools if t.get("des") is not None])
    ...
    def start_detection(self):
        if self.detector_engine is None:
            ...
            return
        valid = [t for t in self.tools if t.get("des") is not None]
        if not valid:
            messagebox.showwarning(
                "警告",
                "没有任何模板特征可用。请检查 data/smart_tools.json 和 "
                "data/smart_templates/ 下的模板图。"
            )
            return
    ...
    def speak_missing(self):
        """播报当前还缺失的工具（基于可见性窗口）"""
        all_tools = set(t["name"] for t in self.tools if t.get("des") is not None)
        now = time.time()
        currently_visible = {
